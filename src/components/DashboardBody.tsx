...
const DashboardBody = ({ dragTarget, dragTargetType, backgroudVisible, isPreview }: MyComponentProps) => {
  let draggingTop = 0;
  let draggingLeft = 0;
  ...
  const [livePreviewComponent, setLivePreviewComponent] = useState<resizePosition>();
  ...
  const handleTileDrop = (event: DragEvent) => {
    ...
    setComponentPositions((prevComponentPositions) => [
      ...
      {
        // prevComponentPositions.length 말고 다른거로 바꿔야함
        id: `${prevComponentPositions.length}-${dragTargetType}`,
        top: draggingTop,
        left: draggingLeft,
        width: 535,
        height: 355,
  ...
  const handleMouseDown = (event: React.MouseEvent) => {
    ...
    // 리사이즈 핸들일때
      ...
      console.log(cardInDashboard);

      const findComponent = componentPositions.find((com) => cardInDashboard.className.includes(com.id));

      const filteredComponent = componentPositions.filter((com) => cardInDashboard.className.includes(com.id));
      ...
      const resizeMouseMove = (moveEvent: MouseEvent) => {
        ...
        console.log(Math.ceil(finalResizeWidth / 90) * 90 - 3);
        console.log(Math.ceil(finalResizeHeight / 90) * 90 - 3);
